...
// Created:     2005/05/12
// RCS-ID:      $Id: SVGCanvasCairo.cpp,v 1.3 2011-06-23 11:29:22 ntalex Exp $
...
void wxSVGCanvasCairo::Init(int width, int height) {
	...
	m_surface = cairo_image_surface_create_for_data(m_data, CAIRO_FORMAT_RGB24,
...
void wxSVGCanvasCairo::Clear(wxRGBColor color) {
	if (!m_cr || !m_surface)
		return;
	cairo_set_source_rgb(m_cr, color.Red() / 255.0, color.Green() / 255.0,
			color.Blue() / 255.0);
