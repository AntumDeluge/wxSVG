...
// Created:     2005/09/27
// RCS-ID:      $Id: SVGPathSegList.cpp,v 1.6 2009-12-10 20:53:14 ntalex Exp $
...
void wxSVGPathSegList::SetValueAsString(const wxString& value)
{
  ...
  const wxString s_numeric = wxT("+-0123456789.Ee");
  const wxString s_separators = wxT(" ,\t\n\r");
  wxChar type = 0;
  double number;
  wxSVGNumberList numbers;
  wxString val = value;
  while (val.Length() && s_separators.Contains(val.Left(1)))
    val.Remove(0,1);
  while (val.Length())
  {
    if (!s_commands.Contains(val.Left(1)))
      break;
    type = val.GetChar(0);
    val.Remove(0,1);
    ...
    while (val.Length())
    {
      ...
      unsigned int pos;
      if (val.Length()>0 && s_numeric.Contains(val.Left(1)))
      {
        pos = 1;
        while (val.Length()>pos && s_numeric.Contains(val.Mid(pos,1)))
          pos++;
        if (!val.Mid(0,pos).ToDouble(&number))
        	wxLogError(wxT("%s = %f"), val.Mid(0,pos).c_str(), number);
        numbers.Add(number);
        val.Remove(0,pos);
      }
      else
