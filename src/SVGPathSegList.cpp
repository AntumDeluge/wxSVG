...
// Created:     2005/09/27
// RCS-ID:      $Id: SVGPathSegList.cpp,v 1.7 2009-12-10 21:13:04 ntalex Exp $
...
inline bool isNumeric(wxChar ch) {
	return ch == wxT('+') || ch == wxT('-') || ch == wxT('.') || ch == wxT('E') || ch == wxT('e')
		|| ch >= wxT('0') && ch <= wxT('9');
}
...
void wxSVGPathSegList::SetValueAsString(const wxString& value)
{
  ...
  while (val.Length())
  {
    ...
    while (val.Length())
    {
      ...
      unsigned int pos;
      if (val.Length()>0 && isNumeric(val[0]))
      {
        pos = 1;
        while (val.Length()>pos && isNumeric(val[pos]))
          pos++;
